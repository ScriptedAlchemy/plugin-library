...
/** One plugin directory, before any file inside it has been read. */
function candidate({ marketplace, slug, version, root, installedMarker }) {
  ...
  for (const marketplace of listDirs(cacheRoot)) {
    for (const slug of listDirs(path.join(cacheRoot, marketplace))) {
      ...
      for (const version of listDirs(pluginDir).filter((v) => !v.startsWith("."))) {
        ...
        out.push(candidate({ marketplace, slug, version, root, installedMarker: fs.existsSync(`${root}.installed`) }));
      }
    }
  }
  return out;
...
function scanLocal(localRoot) {
  return listDirs(localRoot)
    .filter((name) => !name.startsWith("."))
    .map((name) =>
      candidate({ marketplace: LOCAL_MARKETPLACE, slug: name, version: "local", root: path.join(localRoot, name), installedMarker: true }),
    );
}
...
function indexLocalPlugins() {
  ...
  const candidates = [...scanCache(CACHE_ROOT), ...scanLocal(LOCAL_ROOT)];
